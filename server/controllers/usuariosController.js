...
const usuariosController = {
      ...
      for (let i = 0; i < usuarios.length; i++) {
        ...
        if (contrasena) {
          if (usuarioExiste) {
            resultados.push({
              success: false,
              error: `El usuario con DNI '${dni}' ya está registrado`,
              response: null,
            })
          } else {
          ...
          resultados.push({
            success: false,
            error: `La contraseña está ausente en el usuario con DNI '${dni}'`,
            response: null,
          })
        }
      }
  ...
  signIn: async (req, res) => {
    ...
    try {
      ...
      const contrasenaSuccess = bcryptjs.compareSync(
        contrasena,
        dniExist.contrasena
      )
